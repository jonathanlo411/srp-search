...
export function parseTimingResponse(
  ...
  for (let i = TABLE_CONFIG['timing'].start; i < tableItems.length; i += stageSkip) {
    if (tableItems[i].textContent?.includes(name)) {
      ...
      if (tableItems[i + 4].textContent) { raceItem.s3 = tableItems[i + 4].textContent!.slice(1)}
      if (tableItems[i + 5].textContent) { raceItem.s4 = tableItems[i + 5].textContent!.slice(1)}
